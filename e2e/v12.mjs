...
await step('a seeded compound opens with the app’s own protocol', async () => {
  ...
  const vals = await modal().locator('input[type="number"]').evaluateAll((els) => els.map((e) => e.value))
...
await step('a reference-only compound gets a ladder built from its stated range', async () => {
  ...
  const vals = await wiz('input[type="number"]').evaluateAll((els) => els.map((e) => e.value))
