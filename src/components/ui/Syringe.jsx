  ...
  return (
    <div className="w-full overflow-x-auto">
      <svg viewBox="0 0 340 84" className="w-full" style={{ maxWidth: 480 }}>
        {/* needle */}
        <rect x="2" y="40" width="22" height="2.5" rx="1" fill="var(--muted)" />
        {/* barrel */}
        <rect x={barrelX} y="26" width={barrelW} height="30" rx="6" fill="var(--surface2)" stroke="var(--border)" />
        {/* fill */}
        <motion.rect
          x={barrelX} y="26" height="30" rx="6"
          fill={over ? 'var(--coral)' : 'var(--lime)'} opacity="0.55"
          initial={false}
          animate={{ width: fillW }}
        ...
        {/* draw line */}
        <motion.line
          y1="20" y2="62" stroke={over ? 'var(--coral)' : 'var(--lime)'} strokeWidth="2.5"
          initial={false}
        ...
        {Array.from({ length: 21 }, (_, i) => {
          ...
          return (
            <g key={i}>
              <line x1={x} y1={major ? 30 : 33} x2={x} y2={major ? 42 : 39} stroke="var(--muted)" strokeWidth="1" opacity="0.7" />
              {major && (
                <text x={x} y="70" textAnchor="middle" fontSize="8.5" fill="var(--muted)" fontWeight="600">
                  {i * 5}
                </text>
              )}
            </g>
        ...
        <motion.g initial={false} animate={{ x: fillW }} transition={{ type: 'spring', stiffness: 70, damping: 16 }}>
          ...
          <rect x={barrelX + 6} y="38" width={barrelW * 0.28} height="6" rx="3" fill="var(--muted)" opacity="0.5" />
        </motion.g>
      </svg>
      {over && (
        <p className="mt-1 text-xs font-semibold" style={{ color: 'var(--coral)' }}>
