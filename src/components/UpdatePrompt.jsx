    ...
    <AnimatePresence>
      {applyUpdate && (
        <motion.div
          ...
          className="fixed inset-x-4 z-[85] flex items-center gap-3 rounded-[14px] p-3 shadow-lg"
          style={{ bottom: 'calc(var(--nav-h, 92px) + 10px)', background: 'var(--surface)', border: '1px solid var(--border)' }}
        >
          <RefreshCw size={17} className="shrink-0" style={{ color: 'var(--good)' }} />
          ...
          <button onClick={() => setApplyUpdate(null)}
            className="rounded-full px-3 py-2 text-xs font-bold" style={{ background: 'var(--surface-sunk)' }}>
            Later
          </button>
        </motion.div>
      )}
      ...
      {offlineReady && !applyUpdate && (
        <motion.div
          key="offline"
          initial={{ y: 70, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 70, opacity: 0 }}
          className="fixed inset-x-4 bottom-[86px] z-[85] flex items-center gap-2 rounded-[14px] p-3"
          style={{ background: 'var(--surface)', border: '1px solid var(--border)' }}
        >
          <WifiOff size={15} style={{ color: 'var(--text-2)' }} />
